...
The Secrets section reconciles the vault against the profiles jit can see: every stored secret is wired here (a project-local profile uses it), managed elsewhere (referenced only by a global profile or a mount), or unreferenced (a candidate orphan). Add --secrets to expand it into the full per-group listing, the successor to the deprecated jit profile list.
