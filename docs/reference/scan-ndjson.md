...
| `archived` | the file sits under an archived/backup-looking directory (archive, backup, .trash, …); a flag to help you triage - name such a file (or its folder) explicitly in `jit migrate <path>` to convert it. Trash paths set this flag too but the human report gives them their own remedy (`jit migrate --clean`, which finishes the deletion with an encrypted, undoable backup) instead of a migrate offer: migrating a file the user already decided to delete would preserve what deletion is about to fix (schema 0.7.0+) |
...
| `agent`, `cache_area` | on a finding that sits in an AI coding agent's cache or credential store: the agent's name ("Claude Code", "Cline") and what that part of its cache holds ("edit history", "pasted text", "transcripts"; absent for a store with no area). Both were already in `evidence` as prose; as fields a consumer groups the copies per agent the way the human report's "AI Agent Caches" section does (schema 0.21.0+). Set on an `agent_cached_secret` (a copy of a credential confirmed elsewhere) and on an `exposed_secret` found by content inside a cache (schema 0.22.0+): the second is how a cached copy stays visible after its origin is protected - once the `.env` holds a `jit://vault/` pointer there is no value to cross-reference, so the copy is found by its vendor format instead, at the cache walk's 64 MiB bound. When the origin is still in plaintext the cross-reference's copy is reported and the content match for the same value is dropped, so one copy is one finding |
...
| `deep` | `true` when the run was a deep scan (`jit scan --deep`): the vault's secrets were searched for as exact strings, and `vault_copy` findings can appear. Absent on a regular scan (schema 0.23.0+) |
| `vault_secrets_checked` | how many vault secrets a deep scan looked for, after leaving out values the exact-string index will not search for (too short, all digits) and, since 0.24.0, the entries the scan's own name/value gates read as configuration. Absent on a regular scan (schema 0.23.0+) |
| `vault_config_skipped` | how many vault entries a deep scan did **not** search for because they read as configuration rather than credentials - a bare endpoint URL, a filesystem path, a documented-public id. `jit migrate` vaults every variable of a `.env`, ordinary settings included, so a vault is routinely half configuration and searching the machine for copies of an endpoint is noise, not exposure. The same `NonSecretValueReason` / `NonSecretNameReason` gates every other scanner asks; a URL carrying userinfo or an opaque segment (a `postgres://user:pass@host` connection string, a webhook with a token in its path) is **not** excused and stays in the search. `--unfiltered` searches for them all, reports this as `0`, and tags each copy `unfiltered_only` with the rule that would have hidden it. Absent on a regular scan and on a deep scan of a vault that holds only credentials (schema 0.24.0+) |
