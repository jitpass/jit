...
| **Credential Files** | `~/.aws/credentials`, `~/.kube/config`, `.npmrc` auth tokens, the Terraform Cloud token file, Docker registry logins in `~/.docker/config.json`, GCP application-default credentials | [`jit migrate`](../migrate/index.md) |
