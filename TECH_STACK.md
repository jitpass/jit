...
| `golang.org/x/crypto/hkdf` | **Not used.** Planned for deriving a key-wrapping key from a Secure Enclave ECDH secret. The shipped seal (2026-09-25) is Apple's own ECIES, `kSecKeyAlgorithmECIESEncryptionCofactorVariableIVX963SHA256AESGCM`, whose X9.63 SHA-256 derivation and AES-GCM run inside `Security.framework`, so jit never handles a raw shared secret (`internal/secureenclave`). |
...
- **Apple's Secure Enclave only supports NIST P-256** (`kSecAttrKeyTypeECSECPrimeRandom`) for enclave-resident key generation and ECDH, it does not support Curve25519. Pillar II's device-local MEK is therefore necessarily P-256. As built, the MEK is sealed to the enclave key with Apple's ECIES (P-256, X9.63 SHA-256, AES-GCM; §2.2), not a hand-rolled ECDH → HKDF → AEAD.
...
| Code signing | Apple Developer ID Application certificate + `codesign`. **In place since 2026-07-30:** releases are signed as `Developer ID Application: Meni Tasa (CZC6BH93GJ)` (G2 sub-CA, expires 2031; Individual enrollment, so the signature carries a personal name, not an org). Signing was **required for two separate reasons**, not one: (1) Gatekeeper will block an unsigned/unnotarized binary from running at all on a clean macOS install, the distribution reason this was originally scoped for; (2) confirmed by the Secure Enclave spike (`spike/secure-enclave/FINDINGS.md`), persisting a Secure-Enclave-backed key to the keychain fails with `errSecMissingEntitlement` under ad-hoc signing, so a real Team ID is a prerequisite, **but not sufficient on its own.** A free Apple ID "Personal Team" cert (obtained and tested 2026-07-11) clears the ad-hoc gate yet the persistent-key path *still* fails `-34018`: it additionally needs the `application-identifier`/`keychain-access-groups` entitlement authorized by a **provisioning profile**, which per Apple's official ["Signing a daemon with a restricted entitlement"](https://developer.apple.com/documentation/xcode/signing-a-daemon-with-a-restricted-entitlement) can only be embedded in an `.app` bundle (`<Bundle>.app/Contents/embedded.provisionprofile`), never a bare CLI binary. **So SE persistence cannot work through `go install` / Homebrew-formula distribution at all**, see `spike/secure-enclave/FINDINGS.md`'s 2026-07-11 update for the full test log. Actual SE use would require the paid Developer ID Application certificate + notarization + an `.app`-wrapped bundle (the jit-agent is the natural component to bundle). The certificate (2026-07-30) and notarization (v0.80.0) both exist now. **Checked 2026-09-23:** the agent already runs as `JitPass.app/Contents/MacOS/jit` from the launchd plist, but the bundle carries no `embedded.provisionprofile` and no keychain entitlement, so the gap is a signed helper bundle, not a relocation. **Built 2026-09-25, not yet released:** jit-app #52 moves jit into `Contents/Helpers/JitPassAgent.app` (code identifier kept `jit`, so existing keychain keys stay readable, S3f) and #53 signs it with the entitlement and a Developer ID profile. Until that ships, every vault stays on `internal/keychainwrap`. What the move changes for grants, and the two-keys-one-flag rule (MEK with the biometry flag, grant key without), is written down in `design/standing-grants.md`'s last section so it is not relearned. |
| Notarization | goreleaser's `notarize.macos` (Anchore quill): signs with the Developer ID `.p12` and submits to Apple's notary service with `wait: true`. **Live since v0.80.0 (2026-08-07).** It was switched OFF for a stretch because this account's notary was unreliable (1 of 9 submissions Accepted, the rest hanging "In Progress" for hours), which held releases hostage for days; `spike/notarize-e2e/FINDINGS.md` set a numeric gate for turning it back on and records the runs that met it (three consecutive Accepted across two days, verdicts in ~20s; v0.80.0's took 33s). `wait: true` is deliberate — v0.70.1 is the argument against fire-and-forget — with the timeout documented as a hard JWT ceiling, not a tuning knob. **Not stapled** — `xcrun stapler` only staples bundles/dmg/pkg, never a bare Mach-O, so Gatekeeper validates the ticket with an online lookup on first run instead. |
| Distribution channel | Homebrew (`brew install jitpass/tap/jitpass`) and the signed release tarball (curl) + `jit upgrade`. `goreleaser` publishes the tap **cask** — not a formula; `brews` is deprecated for precompiled binaries as of goreleaser v2.10 — from the same release config. The cask carries **no `xattr -dr com.apple.quarantine` hook**, deliberately: Homebrew quarantines its downloads, and now that releases are notarized Gatekeeper clears them against the online ticket, so stripping quarantine would discard the check notarization pays for (and Apple has signalled it may close that bypass). The cask was gated on notarization for exactly this reason and returned with it in v0.80.0. `jit upgrade` handles a Caskroom-resolved copy by telling it to reinstall from the tarball; it does NOT defer to `brew upgrade`. |
...
Vault/keychain access to `LocalAuthentication`/`Security.framework` is a direct CGo/Objective-C bridge (`internal/keychainwrap/*.m`/`*.h`, and `internal/secureenclave/enclave.m`/`.h` for the Secure Enclave) rather than a Go wrapper package, see §2.3.

**Planned here but never adopted, deliberately, not an oversight:**

| Package | Originally planned for | Why it didn't happen |
|---|---|---|
| `github.com/keybase/go-keychain` | Keychain / Secure Enclave access | `internal/keychainwrap` and `internal/secureenclave` bridge `Security.framework`/`LocalAuthentication` directly via CGo instead, see §2.3's own reasoning. |
