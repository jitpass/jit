...
func TestDiscoverEveryLauncherKind(t *testing.T) {
	...
	if strings.Join(pointers, ",") != ".env.bak->security-ops/API,.clisso.yaml->wrap-clisso/blockaid-client-secret" {
...
func TestDiscoverMissingPointerTarget(t *testing.T) {
	...
	if !m.PointersChecked || len(m.MissingPointers) != 1 ||
		m.MissingPointers[0].VaultPath != "wrap-clisso/blockaid-client-secret" ||
