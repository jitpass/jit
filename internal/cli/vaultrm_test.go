...
func TestVaultRmRefusesPointerFile(t *testing.T) {
	...
	withFixtureCwd(t)
	h := newRmHarness(t)
	v := seedSecrets(t, "wrap-clisso/acme-client-secret")
	writeFileAt(t, migrate.ClissoConfigPath(home),
		"providers:\n  acme:\n    client-id: abc\n    client-secret: jit://vault/wrap-clisso/acme-client-secret\n")

	out, err := h.run("vault", "rm", "-y", "wrap-clisso/acme-client-secret")
	if err == nil {
	...
	for _, want := range []string{
		...
		"jit vault rm: nothing deleted, 1 secret is in use",
		glyphAction + " jit vault rm --break-profiles wrap-clisso/acme-client-secret",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("output missing %q, got:\n%s", want, out)
		}
	...
	assertStored(t, v, true, "wrap-clisso/acme-client-secret")
