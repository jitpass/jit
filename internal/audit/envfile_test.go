...
DESCOPE_MGMT_KEY=FAKEjdsiPrdToXFDUxPZm8L49qWYTWnVz0Cji2IbjxHxrpx6WBIgZZbtqE1rS6KY8be0KcY
`)
	findings, err := ScanEnvFiles(Config{HomeDir: home})
	if err != nil {
		t.Fatalf("ScanEnvFiles: %v", err)
...
// carried a cluster of acme-platform .env files holding nothing but
...
// id.acme.io. Vite's own docs say these are bundled into client source at
...
func TestScanEnvFilesPublicBuildVarsNotEscalated(t *testing.T) {
	...
	writeFile(t, filepath.Join(home, "code", "platform", ".env"), `VITE_AUTH0_DOMAIN=id.acme.io
...
func TestScanEnvFilesUnfilteredRevealsSuppressed(t *testing.T) {
	write := func(home string) {
		mkdirAll(t, filepath.Join(home, "app"))
		writeFile(t, filepath.Join(home, "app", ".env"), `VITE_AUTH0_DOMAIN=id.acme.io
