...
**AI jobs run the command for the caller; the caller never holds a key** (`design/agent-jobs.md`, the design doc to read first). A job is an approved command: `internal/job` is the pure half (record, `jobs.json`, fingerprint, the argv check, the output masker) and decrypts nothing; the fingerprint covers what the program loads from outside the folder too (a Python's installation, Node's search folders, every linked dylib: `interp.go`, `libs.go`), read from files and never by running the interpreter, and a program it cannot cover may not be approved `--ask never`; `internal/agent/job.go` decides (fingerprint and rotation re-checked before the prompt, after it, and after the run; stops are sticky until re-approval; one run per job at a time); `internal/cli/jobrunner.go` is the only code that puts secrets into a child's environment, a scratch one, in its own process group, with stdout and stderr through the masker. `internal/mcp` (`jit mcp`) is a hand-rolled stdio MCP server that is a plain socket client: it holds no key and decides nothing, which is why a job run through Claude Desktop or Cursor and one run with `jit job run` are the same RPC. Three rules that are easy to break: every check approval makes lives in `prepareJob`, shared by `job_allow` and `job_preview`, so the app's sheet can never promise what approval refuses; the Touch ID sentence names what the service resolved (folder/program, vault group, how many shown), never the caller-chosen job name; and a job's own `.env` read gets the inert pointer, not the decoy, only for readers descended from that job's registered process (`cli/mountjobs.go`). Every safeguard has a test that was run against the code without it; keep that up when changing one.

### The CGo seam

**Five** darwin-only packages `import "C"` and are the entire non-pure-Go surface, the first place a security reviewer looks: `internal/keychainwrap`, `internal/secureenclave` (the MEK sealed to a Secure Enclave key; `enclave.m` is its whole C surface), `internal/lineage` (libproc, FIFO reader identification — audit logging only, never a gate, because a fast-closing reader can evade it), `internal/pasteboard` (plaintext secrets onto NSPasteboard) and `internal/screenlock` (the lock/sleep trigger that drops the session). `secureenclave` was long counted here while it held only a `doc.go`, which left `pasteboard` and `screenlock` outside the surface reviewers were told to focus on; it has C of its own since plan step B1.

`internal/secureenclave` works **only from a bundle's main executable** carrying an `embedded.provisionprofile` that authorizes the vault's keychain access group: persistence fails `-34018` without the entitlement, and a bare binary or a second executable in a bundle is killed at launch (exit 137). It was never a certificate problem; the July 2026 wall was the missing profile (`spike/secure-enclave-mek/FINDINGS.md`, S3a, 2026-09-25). A plain `go test` exercises it against a fake enclave; `scripts/se-test.sh` wraps and signs the test binary to run the hardware half. `internal/keystore` chooses it for a vault whose root holds `vault-key.sealed`; only `jit vault rekey --wrapper secure-enclave` writes one (shown since v2.3.0, whose JitPass carries the signed helper; `design/secure-enclave-plan.md`).

### Platform scope

**macOS-only by decision** (Apple Silicon), not by accident. A full review found no technical blocker to a Linux port — `GOOS=linux go build ./...` fails on only `internal/lineage`, `internal/keychainwrap`, `internal/secureenclave` and `internal/keystore` (measured 2026-09-25) — but the support-matrix cost is permanent while the demand is hypothetical. Don't propose cross-platform work, and specifically do **not** do the "extract five platform interfaces" refactor: it is speculative abstraction with one implementation per seam, which `TECH_STACK.md` §3 warns against. The existing discipline (isolated CGo packages, `vault.KeyWrapper`, portable `internal/mount` and `internal/audit`) already is the portability investment.
...
Distribution is Homebrew (`brew install jitpass/tap/jitpass`, cask in `jitpass/homebrew-tap`) plus the signed release tarball (curl) + `jit upgrade` — `upgrade.go` tells a Caskroom-resolved copy, or one inside JitPass.app, to use `brew upgrade` rather than self-replace. **Since phase 2 of `design/menu-bar-app.md` the `jitpass` cask is written by the JitPass app release (`jitpass/jit-app`), not by goreleaser here**: the app bundles this repo's tarball at the version pinned in its `jit.version` and symlinks the bundled `jit` onto PATH, so a jit release reaches Homebrew users only when the app bumps that pin and releases. Do not add a `homebrew_casks` block back; it would overwrite the app's cask with a CLI-only one. Four things to know before touching this path:
